...
export default function App() {
  ...
  useEffect(() => {
    ...
    switch (activeMode) {
      case 'kuadrat':
        ...
        break;
      case 'substitusi':
        setInputs({ a: 1, b: 3, c: -4, x: -2 }); 
        break;
      case 'invers':
        setInputs({ a: 3, b: -4, c: 0, x: 5 }); 
        break;
      case 'komposisi':
        setInputs({ a: 2, b: 0, c: 1, x: 3 }); 
        break;
    }
  }, [activeMode]);
  ...
  const renderInputs = () => {
    // Ubah text-base menjadi text-sm (14px) untuk mobile, md:text-lg untuk desktop
    const inputStyle = "w-12 md:w-16 text-center bg-gray-100 border-b-2 border-blue-400 focus:outline-none focus:border-blue-600 focus:bg-blue-50 text-sm md:text-lg font-bold rounded-t-md mx-1 md:mx-2 py-1 transition-colors";
    ...
    return (
      <div className="bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-gray-100 mb-6 flex flex-col items-center justify-center w-full overflow-hidden">
        <div className="w-full overflow-x-auto scrollbar-hide pb-2">
          {/* Ubah text-base menjadi text-sm untuk mobile */}
          <div className="flex items-center justify-center text-sm md:text-xl font-mono whitespace-nowrap text-gray-800 min-w-max mx-auto px-4">
        ...
        {/* Ubah text-base menjadi text-sm untuk mobile */}
        <div className="mt-4 md:mt-6 flex items-center text-sm md:text-lg font-mono flex-wrap justify-center text-center gap-2">
  ...
  const renderSolution = () => {
    ...
    if (activeMode === 'kuadrat') {
      ...
      content = (
        <>
          {/* Tambahkan text-sm md:text-base di sini */}
          <div className="text-gray-700 font-mono space-y-2 text-sm md:text-base">
          ...
          <div className="mt-4 p-4 bg-blue-100 rounded-2xl border-l-4 border-blue-500">
            <p className="font-bold text-blue-800 text-sm md:text-base">Hasil akhirnya adalah: {hasil}</p>
          </div>
        </>
      );
    } 
      ...
      content = (
        <>
          ...
          <div className="mt-4 p-4 bg-green-100 rounded-2xl border-l-4 border-green-500">
            <p className="font-bold text-green-800 text-sm md:text-base">Nilai dari f({vX}) adalah = {hasil}</p>
          </div>
        </>
      );
    }
      ...
      content = (
        <>
          ...
          <div className="mt-4 p-4 bg-orange-100 rounded-2xl border-l-4 border-orange-500">
            <p className="font-bold text-orange-800 text-sm md:text-base">Hasil akhir (f o g)({vX}) adalah {hasil_fog}</p>
          </div>
        </>
      );
    }
    ...
    return (
      <div className="bg-gray-50 rounded-3xl p-6 md:p-8 shadow-inner animate-[fadeUp_0.8s_ease-out_forwards] opacity-0" style={{ animationFillMode: 'forwards' }}>
        <h3 className="text-base md:text-lg font-bold text-gray-800 mb-4 border-b pb-2 flex items-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 text-gray-800 font-sans p-4 md:p-8 flex flex-col items-center relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl mb-6 md:mb-8 relative z-10">
        ...
        <div className="bg-white/40 backdrop-blur-md border border-white/60 p-1.5 md:rounded-full rounded-3xl flex overflow-x-auto scrollbar-hide shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] w-fit max-w-full mx-auto relative snap-x snap-mandatory">
          {modes.map(mode => (
            <button
              key={mode.id}
